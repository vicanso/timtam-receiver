...
class FileStream {
  constructor(logPath, tag) {
    ...
    this.checkFileName();
    this.buf = [];
    this._waitToClose = _.throttle(() => this.close(), waitToCloseInterval);
    this._flush = _.debounce(this.flush.bind(this), flushInterval);
  }
  ...
  checkFileName() {
    ...
    _.delay(this.checkFileName.bind(this), checkFileNameInterval);
  }
  close() {
    if (this.stream) {
      this.stream.end();
